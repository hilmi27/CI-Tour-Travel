<?php
/* 
 * Generated by CRUDigniter v3.2 
 * www.crudigniter.com
 */
 
class Penginapan extends CI_Controller{
    function __construct()
    {
        parent::__construct();
        $this->load->model('Penginapan_model');
    }

    function index($offset=0)
    {

        $penginapan = $this->db->get("penginapan");


        $config['total_rows']=$penginapan->num_rows();
        $config['base_url']= base_url().'Penginapan/index';
        $config['per_page']= 6;


        $config['full_tag_open']="<ul class='pagination pagination-sm' style='position:relative; top: -25px'> ";
        $config['full_tag_close']="</ul>";
        $config['num_tag_open']="<li>";
        $config['num_tag_closen']="</li>";
        $config['cur_tag_open']="<li class='disabled'><li class='active'><a href='#'>";
        $config['cur_tag_close']="<span class='sr-only'></span></a></li>";
        $config['next_tag_open']="<li>";
        $config['next_tag_close']="</li>";
        $config['prev_tag_open']="<li>";
        $config['prev_tag_close']="</li>";
        $config['first_tag_open']="<li>";
        $config['first_tag_close']="</li>";
        $config['last_tag_open']="<li>";
        $config['last_tag_close']="</li>";

        $this->pagination->initialize($config);
        
        $data['halaman']=$this->pagination->create_links();
        $data['offset']=$offset;

        $data['data']=$this->Penginapan_model->ambil_data($config['per_page'],$offset);

    	$data['penginapan'] = $this->Penginapan_model->get_all_penginapan();
        $data['_view'] = 'layouts/front/penginapan';
        $this->load->view('layouts/front/main',$data);
    }

    function caripenginapan(){

    $keyword = $this->input->get('cari', TRUE); //mengambil nilai dari form input cari
    $data['penginapan'] = $this->Penginapan_model->caripenginapan($keyword); //mencari data karyawan berdasarkan inputan
    $data['_view'] = 'layouts/front/caripenginapan';
    $this->load->view('layouts/front/main', $data); //menampilkan data yang sudah dicari
    }
}
